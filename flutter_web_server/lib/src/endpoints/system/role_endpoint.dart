...
class RoleEndpoint extends Endpoint {
  ...
  /// [nickname] 昵称关键词（模糊匹配）
  /// 返回值：分页用户列表
  Future<CommonResponse> getRoleUsers(Session session, int roleId, {int pageNum = 1, int pageSize = 20, String? nickname}) async {
    try {
      final authInfo = session.authenticated;
      if (authInfo == null) {
        return CommonResponse(code: ResultCode.failed.code, message: '未登录');
      ...
      final safePageNum = pageNum < 1 ? 1 : pageNum;
