...
class AuthService {
  ...
  static Future<CommonResponse> login(Session session, String username, String password) async {
    try {
      ...
      // scopes 使用 Serverpod 标准集合：Scope.none 表示默认登录权限，tenantId:xxx 用于解析租户隔离。
      final tokenScopes = <Scope>{Scope.none};
      if (user.tenantId > 0) {
        tokenScopes.add(Scope('tenantId:${user.tenantId}'));
      }
  ...
  static Future<CommonResponse> refreshToken(Session session, String refreshToken) async {
    try {
