...
//   ///
//   /// 未匹配到昵称时返回 null。
//   static Future<String?> getNicknameByUserIdentifier(Session session, String? userIdentifier) async {
...
//   static Future<CommonResponse> add(Session session, UserRequest req) async {
//     try {
//       // 1. 仅允许已登录用户创建新用户
...
//   /// [req] 用户列表查询参数
//   /// 返回值：用户列表
//   static Future<CommonResponse> getList(Session session, UserListRequest req) async {
//     try {
//       final authInfo = session.authenticated;
//       if (authInfo == null) {
...
//   static Future<Set<int>> _collectDeptAndChildrenIds(Session session, int rootDeptId, {int? tenantId}) async {
//     final visited = <int>{};
...
//   static Future<CommonResponse> getUserInfo(Session session) async {
//     final authInfo = session.authenticated;
//     if (authInfo == null) {
//       return CommonResponse(code: ResultCode.failed.code, message: '未登录');
//     }
...
//   static Future<CommonResponse> getUserRoutes(Session session) async {
//     try {
//       final authInfo = session.authenticated;
//       if (authInfo == null) {
//         return CommonResponse(code: ResultCode.failed.code, message: '未登录');
...
//   /// [parentId] 父菜单ID
//   /// 返回值：路由树列表
//   static List<Map<String, dynamic>> _buildRouteTree(List<SysMenu> menus, List<String> roleNames, {int parentId = 0}) {
...
//   ///
//   /// [req] 用户信息（需包含 id）
//   static Future<CommonResponse> update(Session session, UserRequest req) async {
//     try {
//       final authInfo = session.authenticated;
//       if (authInfo == null) {
...
//   static Future<void> _saveUserRoles(
//     Session session, {
//     required int userId,
...
//   /// [ids] 用户ID列表
//   /// 返回值：处理结果汇总
//   static Future<CommonResponse> resetPassword(Session session, List<int> ids) async {
//     try {
//       final authInfo = session.authenticated;
//       if (authInfo == null) {
//         return CommonResponse.failed('未登录');
