...
class GiTag extends StatelessWidget {
  ...
  Map<String, dynamic> get _styleConfig {
    ...
    switch (type) {
      case GiTagType.light:
        ...
        config['backgroundColor'] = tagColor.withValues(alpha: 0.1);
      ...
      case GiTagType.lightOutline:
        config['textColor'] = tagColor;
        config['backgroundColor'] = tagColor.withValues(alpha: 0.1);
        config['borderColor'] = tagColor.withValues(alpha: 0.2);
        config['borderWidth'] = 1.0;
        break;
    }
  ...
  Widget _buildCloseButton(Map<String, dynamic> styleConfig) {
    ...
    return _HoverableCircleButton(
      diameter: buttonSize,
      iconSize: iconSize,
      iconColor: styleConfig['textColor'] as Color,
      hoverBackgroundColor: (styleConfig['textColor'] as Color).withValues(alpha: 0.8),
      onTap: onClose,
    );
  }
...
/// 内部组件：带悬停圆形背景的按钮
class _HoverableCircleButton extends StatefulWidget {
  ...
  @override
  State<_HoverableCircleButton> createState() => _HoverableCircleButtonState();
}

class _HoverableCircleButtonState extends State<_HoverableCircleButton> {
  bool _isHovering = false;

  /// 获取悬停时的图标颜色
  Color get _hoverIconColor {
    // 当悬停时，根据背景色调整图标颜色
    if (_isHovering) {
      // 如果背景色较深，使用白色图标；如果背景色较浅，使用更深的颜色
      final luminance = widget.hoverBackgroundColor.computeLuminance();
      if (luminance < 0.5) {
        return Colors.white;
      } else {
        return widget.iconColor.withValues(alpha: 0.8);
      }
    }
    ...
    return MouseRegion(
      ...
      child: GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: _isHovering ? widget.hoverBackgroundColor : Colors.transparent,
            borderRadius: BorderRadius.circular(widget.diameter / 2),
          ),
          child: AnimatedDefaultTextStyle(
            duration: const Duration(milliseconds: 120),
            style: TextStyle(color: _hoverIconColor),
            child: Icon(
              Icons.close,
