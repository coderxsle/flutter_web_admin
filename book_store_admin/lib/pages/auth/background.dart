...
class _ServerpodBackgroundState extends State<ServerpodBackground> with TickerProviderStateMixin {
  ...
  void _generateStaticBubbles() {
    ...
    for (var i = 0; i < 25; i++) {
      ...
      // 静态气泡大小变化更大一些
      final radius = 5.0 + _random.nextDouble() * 35.0;
  ...
  Bubble _createBubble(Size screenSize) {
    ...
    final radius = 10.0 + _random.nextDouble() * 30.0;
    final x = _random.nextDouble() * screenSize.width;
    final y = screenSize.height + radius + _random.nextDouble() * 300;
    final color = colors[_random.nextInt(colors.length)];
    final speed = 0.5 + (40 - radius) / 40 * 1.5;
  ...
  Widget build(BuildContext context) {
    ...
    if (_fadeInController == null) {
      return Container(
        color: const Color.fromARGB(255, 221, 163, 245),
        child: widget.child,
      );
    }
    ...
    return Container(
          ...
          decoration: BoxDecoration(
            gradient: RadialGradient(
              ...
              radius: 1.2,
              colors: [
                const Color.fromARGB(30, 222, 168, 247), // 暖白色中心
                const Color.fromARGB(50, 182, 168, 242), // 冷白色边缘
              ],
              stops: const [0.4, 1.0],
            ),
          ),
    ...
    for (var bubble in bubbles) {
      ...
      if (bubble.radius > 10) { // 只给较大的气泡添加高光，避免小气泡看起来奇怪
        ...
        final highlightPaint = Paint()
          ..color = Colors.white.withOpacity(bubble.opacity * 1.5)
          ..style = PaintingStyle.stroke
          ..strokeWidth = 1.0;
