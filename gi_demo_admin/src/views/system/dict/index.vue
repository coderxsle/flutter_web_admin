<template>
  <GiPageLayout margin>
    <a-row justify="space-between" class="gi-row-tool">
      ...
      <a-space wrap>
        <a-input-group>
          <a-select v-model="queryParams.status" :options="dictData.common_status" placeholder="状态" allow-clear
...
import { getDictCodeList, deleteDictCode, list, delete111 } from '@/apis/system/dict'
...
const { loading, tableData: dictList, pagination, selectedKeys, search, select, selectAll, fixed, handleDelete } = useTable({
  // listAPI: (page) => getDictCodeList({ ...page, ...queryParams }),
  listAPI: () => list({ page: 1, pageSize: 10, keyword: queryParams.name }),
  immediate: true
})
...
const onDelete = (item: T.ListItem) => {
  // return handleDelete(() => deleteDictCode({ ids: [item.id] }), { showModal: false })
  return handleDelete(() => delete111(item.id), { showModal: false })
}
...
const onMulDelete = () => {
  ...
  return handleDelete(() => deleteDictCode({ ids: selectedKeys.value as number[] }))
}

const onEdit = (item: T.ListItem) => {
  DictFormModalRef.value?.edit(item)
}
