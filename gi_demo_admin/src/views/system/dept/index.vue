<template>
  <GiPageLayout margin>
    <a-row justify="space-between" class="gi-row-tool">
      ...
      <a-space wrap>
        <a-input-group>
          <a-select v-model="queryParams.status" :options="dictData.common_status" placeholder="部门状态"
...
const onAdd = (parentId?: number) => {
  DeptFormModalRef.value?.add(parentId)
}
  ...
  { title: '创建时间', dataIndex: 'createTime', width: 200 },
  {
    ...
    render: ({ record }) => (
      <Space>
        ...
        {record.parentId && (
          <GiButton type="add" size="mini" status="success" disabled={record.disabled} onClick={() => onAdd(Number(record.id))} />
        )}
